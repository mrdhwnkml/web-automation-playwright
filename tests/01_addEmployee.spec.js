...
test('User add Employee should be success', async ({ page, context, login }) => {
    ...
    await page.locator('(//input[@class="oxd-input oxd-input--active"])[2]').click()
    await page.keyboard.press('Control+A')
    await page.keyboard.press('Delete')
    await page.locator('//input[@class="oxd-input oxd-input--focus"]').fill(userData.employeeId)

    //Create login detail
    await page.locator('form span').click();
    await page.locator('(//input[@class="oxd-input oxd-input--active"])[3]').fill(userData.username)

    // Save to local storage
    await saveToLocalStorageAndFile(page, { key: 'username', value: userData.username });
    ...
    const storedPassword = await page.evaluate(() => localStorage.getItem('password'));
    ...
    const successMessage = page.locator('text=Successfully Saved', { state: 'visible', timeout: 5000 });
